...
subtitle: "HES 505 Fall 2024: Session 21"
...
1. What makes a good park? How should it integrate with other parks?
...
ac <- read.csv(paste(system.file(package="dismo"), '/ex/acaule.csv', sep='')) %>%
  st_as_sf(., coords=c("lon", "lat"), crs = 4326) %>%
  st_transform(., crs = 31970)
ac_buff <- st_buffer(ac, 50000) %>% st_union()
samp1 <- st_sample(ac_buff, 250) %>%
  st_as_sf(., crs=st_crs(ac_buff)) %>%
  mutate(y=0) %>%
  rename(geometry = x)
...
crop_box <- c(xmin = -75, xmax = -71, ymax = -10, ymin = -15)

presabs_crop <- st_crop(presabs, crop_box)

mintempcold <- min(geodata::worldclim_tile("tmin", lat=-12, lon=-73, path=tempfile()))
mintempcold <- crop(mintempcold, presabs_crop)

meananntemp <- mean(geodata::worldclim_tile("tavg", lat=-12, lon=-73, path=tempfile()))
meananntemp <- crop(meananntemp, presabs_crop)

temprange <- max(geodata::worldclim_tile("tmin", lat=-12, lon=-73, path=tempfile())) - min(geodata::worldclim_tile("tmin", lat=-12, lon=-73, path=tempfile()))
temprange <- crop(temprange, presabs_crop)

totalprecip <- sum(geodata::worldclim_tile("prec", lat=-12, lon=-73, path=tempfile()))
totalprecip <- crop(totalprecip, presabs_crop)

precipdrymonth <- min(geodata::worldclim_tile("prec", lat=-12, lon=-73, path=tempfile()))
precipdrymonth <- crop(precipdrymonth, presabs_crop)

precipwetmonth <- max(geodata::worldclim_tile("prec", lat=-12, lon=-73, path=tempfile()))
precipwetmonth <- crop(precipwetmonth, presabs_crop)

pres.abs <- presabs_crop[sample(nrow(presabs_crop), nrow(presabs_crop)),]
pred.stack <- c(meananntemp, totalprecip, precipwetmonth, precipdrymonth, mintempcold, temprange)
names(pred.stack) <- c("MeanAnnTemp", "TotalPrecip", "PrecipWetMonth", "PrecipDryMonth", "MinTempCold", "TempRange")
plot(pred.stack)
```
...
plot(pres.pts$geometry, add=TRUE, pch=3, col="aquamarine", cex=1)
plot(abs.pts$geometry, add=TRUE, pch ="-", col="red", cex=2)
```
:::
:::
...
logistic.rich <- glm(y ~ MeanAnnTemp + PrecipWetMonth + PrecipDryMonth, family=binomial(link="logit"), data=pts.df[,2:8])
```
